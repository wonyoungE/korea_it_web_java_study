...
public class Main {
    public static void main(String[] args) {
        ...
        // 자식은 부모의 멤버 변수와 메서드를 모두 가지고 있으므로 부모 클래스로 형변환해도 잃을 게 없다.
        // Animal 타입을 참조
        // 방법 1 A a = new B();
...
//      animal.wagTail();   // 불가능, 부모 클래스를 참조하기 때문에 자식 클래스에만 있는 메서드는 호출 불가
        ...
        ((Dog)animal).wagTail();    // 일시적 형변환 -> 어디에 저장 안했다.
        ...
        // animal2는 사실 dog의 내용을 가진 Animal 클래스를 참조하는 (업캐스팅) 클래스인데 (=껍데기는 Animal)
        // 얘를 다시 dog 객체에 넣음;; => 다운캐스팅. 그러면 얘의 껍데기는 Animal인가 Dog인가..
        Dog d = (Dog) animal2;
        d.wagTail();
        ...
        // Dog, Cat, Tiger 클래스는 Animal 클래스의 자식.
        Animal dog4 = new Dog();
        Animal cat = new Cat();
